<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:42:"./application/admin\view\school_index.html";i:1508820211;s:42:"./application/admin\view\index_header.html";i:1507643832;s:42:"./application/admin\view\index_footer.html";i:1504421376;}*/ ?>
<!DOCTYPE html>
<html>
<head>  
...
<table width="100%">
	  ...
	  <tr>
		...
		<td align="center">
		<a change="<?php echo $vo['id']; ?>" onclick="changeshows(this);" <?php if($vo['show'] == 1): ?>class="layui-unselect layui-form-switch layui-form-onswitch"<?php else: ?>class="layui-unselect layui-form-switch"<?php endif; ?>>
		<em>显示</em><i></i>
		</a>
		</td>
		<td align="center">
		...
		<td align="center">
		<a change="<?php echo $vo['id']; ?>" onclick="changecommend(this);" <?php if($vo['commend'] == 1): ?>class="layui-unselect layui-form-switch layui-form-onswitch"<?php else: ?>class="layui-unselect layui-form-switch"<?php endif; ?>>
		<em>推荐</em><i></i>
		</a>
		</td>
		<td align="center"><?php echo $vo['view']; ?></td>
		<td align="center"><?php echo date("Y-m-d",$vo['addtime']); ?></td>
        <td align="center"><a class="layui-btn layui-btn-mini layui-btn-warm" href="<?php echo url('school/edit',array('id'=>$vo['id'])); ?>">修改</a> <a class="layui-btn layui-btn-mini layui-btn-danger del_btn" member-id="<?php echo $vo['id']; ?>" title="删除" nickname="<?php echo $vo['title']; ?>">删除</a>
        </td>
      </tr>
...
function changesettop(o){
  ...
  $.ajax({
	  ...
	  dataType:"json",
      data:{change:change},
	  url:"<?php echo url('school/changesettop'); ?>",
...
function changeshows(o){
  ...
  $.ajax({
	  ...
	  dataType:"json",
      data:{change:change},
	  url:"<?php echo url('school/changeshows'); ?>",
	  success:function(data){
		  if(data == 1){
			  $(o).attr("class","layui-unselect layui-form-switch");
	      }else{
...
var change=$(o).attr("change");
  $.ajax({
	  ...
	  dataType:"json",
      data:{change:change},
	  url:"<?php echo url('school/changecommend'); ?>",
	  success:function(data){
		  if(data == 1){
			  $(o).attr("class","layui-unselect layui-form-switch");
	      }else{
...
layui.use('form',function(){
  ...
  jq('.del_btn').click(function(){
    ...
    layer.confirm('确定删除【'+name+'】?', function(index){
      ...
      jq.post('<?php echo url("school/dels"); ?>',{'id':id},function(data){
        if(data.code == 200){
          layer.close(loading);
  ...
  form.on('submit(delete)', function(data){
    ...
    layer.confirm('确定批量删除?', function(index){
      ...
      jq.post('<?php echo url("school/delss"); ?>',param,function(data){
        if(data.code == 200){
          layer.close(loading);
          layer.msg(data.msg, {icon: 1, time: 1000}, function(){
            location.reload();
